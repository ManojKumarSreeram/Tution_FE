...
import { studentSignUp } from '../../Services/ApiService';
  ...
  const validateField = (name, value) => {
    switch (name) {
      ...
      case 'phone_number':
        ...
        if (!/^\d{10,13}$/.test(value)) return 'Phone number must be 10-13 digits';
        break;
      case 'gender':
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateAll()) {
      try {
        ...
        if (result?.status_code === 200 && studentId) {
          toast.success(`Registration successful! Student ID: ${studentId}`, { autoClose: 7000 });
